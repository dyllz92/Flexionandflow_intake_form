(function() {
    ...
    // DOM Elements
    let steps, stepIndicators, prevBtn, nextBtn, submitBtn, validationToast, stepValidationMessageEl, stepStatusEl;
    ...
    function getValidationState(stepNum) {
        ...
        switch (stepNum) {
            case 1: {
                ...
                if (!firstName || !firstName.value.trim()) invalidate(firstName, 'Please enter your first name.');
                else if (!lastName || !lastName.value.trim()) invalidate(lastName, 'Please enter your last name.');
                else if (!email || !email.value.trim()) invalidate(email, 'Please enter your email address.');
                else if (!mobile || !mobile.value.trim()) invalidate(mobile, 'Please enter your phone number.');
                else if (!dateOfBirth || !dateOfBirth.value.trim()) invalidate(dateOfBirth, 'Please enter your date of birth.');
                else if (!isValidDateInput(dateOfBirth)) invalidate(dateOfBirth, 'Please enter your date of birth in DD/MM/YYYY format.');
                break;
            }
            case 2: {
                ...
                if (visitReasonsInputs.length && visitReasons.length === 0) invalidate(visitReasonsInputs[0], 'Please select at least one reason for your visit.');
                    ...
                    invalidate(lastTreatmentDate, 'Please enter your last treatment date in DD/MM/YYYY format.');
                }
                break;
            }
            case 3: {
                const takingMedicationsInputs = document.querySelectorAll('input[name="takingMedications"]');
                const takingMedications = document.querySelectorAll('input[name="takingMedications"]:checked');
                const hasAllergiesInputs = document.querySelectorAll('input[name="hasAllergies"]');
                const hasAllergies = document.querySelectorAll('input[name="hasAllergies"]:checked');
                const hasRecentInjuriesInputs = document.querySelectorAll('input[name="hasRecentInjuries"]');
                ...
                const seenOtherProviderInputs = document.querySelectorAll('input[name="seenOtherProvider"]');
                const seenOtherProvider = document.querySelectorAll('input[name="seenOtherProvider"]:checked');
                const pregnantBreastfeedingInputs = document.querySelectorAll('input[name="pregnantBreastfeeding"]');
                const pregnantBreastfeeding = document.querySelectorAll('input[name="pregnantBreastfeeding"]:checked');

                if (takingMedicationsInputs.length && takingMedications.length === 0) invalidate(takingMedicationsInputs[0], 'Please indicate if you are taking any medications.');
                else if (hasAllergiesInputs.length && hasAllergies.length === 0) invalidate(hasAllergiesInputs[0], 'Please indicate if you have any allergies.');
                else if (hasRecentInjuriesInputs.length && hasRecentInjuries.length === 0) invalidate(hasRecentInjuriesInputs[0], 'Please indicate if you have had recent accidents, injuries or surgeries.');
                else if (medicalConditionsInputs.length && medicalConditions.length === 0) invalidate(medicalConditionsInputs[0], 'Please select at least one option from the medical conditions list.');
                else if (seenOtherProviderInputs.length && seenOtherProvider.length === 0) invalidate(seenOtherProviderInputs[0], 'Please indicate if you have seen another healthcare provider.');
                else if (pregnantBreastfeedingInputs.length && pregnantBreastfeeding.length === 0) invalidate(pregnantBreastfeedingInputs[0], 'Please indicate if you are pregnant or breastfeeding.');
                else {
                    const medicationsYes = document.querySelector('input[name="takingMedications"][value="Yes"]')?.checked;
                    if (medicationsYes) {
                        const medicationsList = document.getElementById('medicationsList');
                        if (!medicationsList || !medicationsList.value.trim()) invalidate(medicationsList, 'Please list your medications.');
                    }

                    const allergiesYes = document.querySelector('input[name="hasAllergies"][value="Yes"]')?.checked;
                    if (allergiesYes) {
                        const allergiesList = document.getElementById('allergiesList');
                        if (!allergiesList || !allergiesList.value.trim()) invalidate(allergiesList, 'Please list your allergies.');
                    }
                }
                break;
            }
            case 4: {
                ...
                if (!consentAll || !consentAll.checked) invalidate(consentAll, 'Please confirm you have read and agreed to the Declaration & Privacy Consent.');
                break;
            }
        }
    ...
    function handleFieldInteraction(event) {
        if (event.target && event.target.closest && event.target.closest('.wizard-step')) {
    ...
    function handleClickInteraction(event) {
        if (event.target && event.target.closest && event.target.closest('.wizard-step')) {
            const isLabelClick = event.target.tagName === 'LABEL' || event.target.closest('label');
            if (isLabelClick) {
                stepTouched[currentStep] = true;
    ...
    function resolveFieldGroup(element) {
        ...
        return element.closest('.form-group') || element.closest('.radio-group') || element.closest('.checkbox-group') || element;
    }

    function clearFieldErrors() {
        document.querySelectorAll('.form-group.field-error').forEach(el => el.classList.remove('field-error'));
    ...
    function updateStepValidationMessage(state) {
        ...
        stepValidationMessageEl.textContent = state.message || 'Please complete all required fields to continue.';
