...
class InteractiveMuscleMap {
  constructor(containerId, marksInputId) {
    ...
    this.canvas = null;
    this.ctx = null;
    this.dotRadius = 32; // Increased from 24 for larger, more visible markers
    this.bodyImage = null;
  ...
  initializeGenderListener() {
    ...
    if (selectedGender) {
      this.currentGender =
        selectedGender.value === "Female" ? "Female" : "Male";
      this.loadBodyImage();
    }
  }
  ...
  loadBodyImage() {
    ...
    candidates.push(
      this.currentGender === "Female"
        ? "/js/Female Body Map.png"
        : "/js/Male Body Map.png",
    );
    // Support "Body Chart" naming with underscores
    candidates.push(
      this.currentGender === "Female"
        ? "/img/Female_Body_Chart.png"
        : "/img/Male_Body_Chart.png",
    );
    candidates.push(
      this.currentGender === "Female"
        ? "/js/Female_Body_Chart.png"
        : "/js/Male_Body_Chart.png",
    );
  ...
  drawBodyMap() {
    ...
    ctx.fillText(
      "Click to mark areas of discomfort",
      centerX,
      canvas.height - 10,
    );
  ...
  redrawDots() {
    ...
    // Draw all dots with enhanced styling
    this.marks.forEach((mark) => {
      ...
      // Draw main dot with gradient effect
      ...
      gradient.addColorStop(0, "#ff6b6b"); // Light red center
      gradient.addColorStop(1, "#e74c3c"); // Darker red edge
      ...
      // Draw cross in center with better contrast
      this.ctx.strokeStyle = "#ffffff";
      this.ctx.lineWidth = 2.5;
      this.ctx.lineCap = "round"; // Rounded line ends for softer look

      this.ctx.beginPath();
      this.ctx.moveTo(mark.x - 5, mark.y);
      this.ctx.lineTo(mark.x + 5, mark.y);
      this.ctx.stroke();

      this.ctx.beginPath();
      this.ctx.moveTo(mark.x, mark.y - 5);
      this.ctx.lineTo(mark.x, mark.y + 5);
      this.ctx.stroke();
    });
  }
